...
RESULTS_PATH = RESULTS_DIR / "extensive_ablation_results.csv"
...
def decompose_series(series, period):
    ...
    stl = STL(series + noise, seasonal=seasonal, trend=trend, period=int(period))
    result = stl.fit()
    return result.resid
...
def df_to_pcmci(df_resid, tau_max=2):
    ...
    # We use fewer neighbors/shuffles strictly to make the extensive run computationally feasible in our environment.
    cmi = CMIknn(significance='shuffle_test', knn=0.1, shuffle_neighbors=3, transform='ranks', workers=-1)
...
def main():
    ...
    lags_list = [2, 4, 6]
    periods_list = [10, 15, 20, 25, 30, 35]
    
    out = open(RESULTS_PATH, 'w')
    out.write("Variables\tSamples\tLag\tPeriod\tTPR\tFDR\tSHD\n")
    out.write("-" * 65 + "\n")
    out.flush()
    print("Variables\tSamples\tLag\tPeriod\tTPR\tFDR\tSHD", flush=True)
    print("-" * 65, flush=True)
        ...
        for n_samples in samples_list:
            ...
            for period in periods_list:
                ...
                for lag in lags_list:
                    ...
                    log_str = f"{n_vars}\t\t{n_samples}\t{lag}\t{period}\t{tpr:.3f}\t{fdr:.3f}\t{shd}"
